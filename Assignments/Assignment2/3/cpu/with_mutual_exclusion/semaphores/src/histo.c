...
void *compute(void *info) {
    ...
    for (uint64_t in = ti->n_start; in < ti->n_stop; ++in) {
        for (uint64_t im = ti->m_start; im < ti->m_stop; ++im) {
            sem_wait(&ti->sems[(*picture)[in][im]]);
            ++ti->histo[(*picture)[in][im]];
            sem_post(&ti->sems[(*picture)[in][im]]);
        }
    }
...
int main(int argc, char *argv[]){
   ...
   if (multiply != 0) {
    ...
    // srand(seed);
    // uint8_t (*picture)[n][m] = malloc(n * m * sizeof(uint8_t));

    // /* Fill the picture with random pixels between 0 and 255 */
    // int64_t number_of_cors = (sysconf(_SC_NPROCESSORS_ONLN) / 2);
    // struct create_info cis[number_of_cors];
    // pthread_t creater_threads[number_of_cors];

    // uint64_t start = 0;
    // uint64_t size =  n / number_of_cors;
    // uint64_t left_over = n % number_of_cors;

    // cis[0].seed = seed;
    // cis[0].n = n;
    // cis[0].m = m;
    // cis[0].n_start = start;
    // cis[0].n_stop = size + left_over;
    // cis[0].picture = (*picture);

    // pthread_create(&creater_threads[0],
    //         NULL,
    //         &create,
    //         &cis[0]);

    // start = size + left_over;
    // for (int64_t idx = 1; idx < number_of_cors; ++idx) {
    //     cis[idx].seed = seed + idx;
    //     cis[idx].n = n;
    //     cis[idx].m = m;
    //     cis[idx].n_start = start;
    //     cis[idx].n_stop = size + start;
    //     cis[idx].picture = (*picture);

    //     start += size;
    //     pthread_create(&creater_threads[idx],
    //             NULL,
    //             &create,
    //             &cis[idx]);
    // }

    // for (int64_t idx = 0; idx < number_of_cors; ++idx) {
    //     pthread_join(creater_threads[idx], NULL);
    // }
    ...
    for (uint8_t idx = 0; idx < 255; ++idx) {
        sem_init(&sems[idx], 0, 1);
    }
    sem_init(&sems[255], 0, 1);
